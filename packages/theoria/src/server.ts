...
async function chooseExecution(id,entryId,preserve=false){const request=++state.detailRequest;const previous=[state.execution,state.entry];const sameExecution=state.execution===id;const observationId=preserve&&sameExecution?state.observation?.id:null;const positions=preserve&&sameExecution?[el('timeline'),el('inspector'),...document.querySelectorAll('#inspector .code')].map(x=>[x.scrollLeft,x.scrollTop]):null;state.execution=id;state.entry=entryId||null;renderExecutions();let timeline,waterfall;try{[timeline,waterfall]=await Promise.all([api('/api/timeline/'+encodeURIComponent(id)),api('/api/waterfall/'+encodeURIComponent(id))])}catch(e){if(request!==state.detailRequest)return;[state.execution,state.entry]=previous;renderExecutions();el('error').textContent=e.message;el('error').style.display='block';return}if(request!==state.detailRequest)return;state.timeline=timeline;state.waterfall=waterfall;state.observation=(observationId?state.timeline.find(x=>x.id===observationId):null)||(entryId?state.timeline.find(x=>x.id===entryId):null)||state.timeline.findLast(x=>x.phase==='failed')||state.timeline.at(-1)||null;el('correlation').textContent=state.timeline[0]?.context?.correlationId?'correlation '+state.timeline[0].context.correlationId.slice(0,8):'';renderDetail();renderInspector();if(positions)[el('timeline'),el('inspector'),...document.querySelectorAll('#inspector .code')].forEach((x,i)=>{if(positions[i])x.scrollTo(...positions[i])})}
