...
async function loadExecutions(resetSelection=false){if(state.loading){if(resetSelection){state.reload=true;state.reset=true;state.listRequest++}return}state.loading=true;const request=++state.listRequest;try{el('error').style.display='none';const p=new URLSearchParams();if(state.kind)p.set('kind',state.kind);if(state.phase)p.set('phase',state.phase);if(state.search)p.set('search',state.search);const executions=await api('/api/entries?'+p);if(request!==state.listRequest)return;state.executions=executions;if(resetSelection){state.execution=null;state.entry=null;state.observation=null}renderExecutions(!resetSelection);if(state.execution)await chooseExecution(state.execution,state.entry||undefined,true);else if(state.executions[0])await chooseExecution(state.executions[0].executionId,state.executions[0].entryId);else{state.timeline=[];state.waterfall=[];el('correlation').textContent='';renderDetail();renderInspector()}}catch(e){el('error').textContent=e.message;el('error').style.display='block'}finally{state.loading=false;if(state.reload){const reset=state.reset;state.reload=false;state.reset=false;loadExecutions(reset)}}}
...
async function chooseExecution(id,entryId,preserve=false){if(!preserve)state.listRequest++;const request=++state.detailRequest;const previous=[state.execution,state.entry];const sameExecution=state.execution===id;state.execution=id;state.entry=entryId||null;renderExecutions();let timeline,waterfall;try{[timeline,waterfall]=await Promise.all([api('/api/timeline/'+encodeURIComponent(id)),api('/api/waterfall/'+encodeURIComponent(id))])}catch(e){if(request!==state.detailRequest)return;[state.execution,state.entry]=previous;renderExecutions();el('error').textContent=e.message;el('error').style.display='block';return}if(request!==state.detailRequest)return;const observationId=preserve&&sameExecution?state.observation?.id:null;const positions=preserve&&sameExecution?[el('timeline'),el('inspector'),...document.querySelectorAll('#inspector .code')].map(x=>[x.scrollLeft,x.scrollTop]):null;state.timeline=timeline;state.waterfall=waterfall;state.observation=(observationId?state.timeline.find(x=>x.id===observationId):null)||(entryId?state.timeline.find(x=>x.id===entryId):null)||state.timeline.findLast(x=>x.phase==='failed')||state.timeline.at(-1)||null;el('correlation').textContent=state.timeline[0]?.context?.correlationId?'correlation '+state.timeline[0].context.correlationId.slice(0,8):'';renderDetail();renderInspector();if(positions)[el('timeline'),el('inspector'),...document.querySelectorAll('#inspector .code')].forEach((x,i)=>{if(positions[i])x.scrollTo(...positions[i])})}
